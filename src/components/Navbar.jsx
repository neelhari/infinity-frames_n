      ...
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-2 sm:py-2.5">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {/* Account / User Icon for Desktop and Tablet */}
            <button
              onClick={() => goTo(isAuthenticated ? '/account' : '/login')}
              className={`p-2 transition-colors rounded-full hover:bg-gray-100 cursor-pointer flex items-center gap-1.5 ${
                location.pathname.startsWith('/account') || location.pathname.startsWith('/login')
                  ? 'text-[#B38029]'
                  : 'text-gray-700 hover:text-[#B38029]'
              }`}
              title={isAuthenticated ? `My Account (${user?.name || 'Customer'})` : 'Sign In / Account'}
            >
